      ...
      <main className="min-h-screen flex items-center justify-center px-4">
        <div className="card-base p-8 text-center max-w-sm w-full">
          ...
          <p className="text-sm text-chalk/50">
            We've sent a confirmation link to <strong className="text-mud">{email}</strong>.
    ...
    <main className="min-h-screen flex items-center justify-center px-4 py-16">
      <div className="card-base p-8 w-full max-w-sm">
        ...
        <button onClick={handleGoogle}
          className="w-full btn-ghost mb-4 py-2.5 border border-chalk/10 justify-center gap-3">
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <button type="submit" disabled={loading} className="btn-brass w-full mt-2 disabled:opacity-50">
            {loading ? 'Loading…' : mode === 'signin' ? 'Sign in' : 'Create account'}
          </button>
        </form>

        <p className="text-center text-xs text-chalk/30 mt-4">
          {mode === 'signin' ? "Don't have an account? " : 'Already have an account? '}
          <button
            onClick={() => { setMode(mode === 'signin' ? 'signup' : 'signin'); setError(null) }}
            className="text-mud hover:text-mud/80 underline underline-offset-2 transition-colors"
          >
